                     ...
                     <div class="p-3 d-flex align-items-center w-100" href="#">
                         <div class="dropdown-list-image mr-3">
                             <img class="rounded-circle"
                                 src="{{ asset('public/frontend/img/user/' . Auth::user()->img) }}" alt="" />
                             <div class="status-indicator bg-success"></div>
                         </div>
                         <!-- ===========[Share Post]=============  -->
                         <div class="w-100">
                             <textarea type="text" name="post" placeholder="Write your thoughts..."
                                 class="form-control border-0 p-0 shadow-none post" autocomplete="off"></textarea>
                         </div>
                     </div>
                 </div>
             </div>
         ...
         @foreach ($book as $data)
             <div class="box shadow-sm border rounded bg-white mb-3 osahan-post">
                 ...
                 <div class="p-3 border-bottom osahan-post-body">
                     ...
                     <hr>
                     <p class="mb-0">
                         {{Str::of($data->main_book)->words(50,'...')}}
                         <a href="{{route('author.bookView',$data->id)}}" class="">Read More</a>
                     </p>
                 </div>
                 <div class="p-3 border-bottom osahan-post-footer overflow-hidden">
                     ...
                     <a href="#" class="mr-3 pl-3 text-secondary emojiArea">
                         <span class="emj_like" ids="{{ $data->id }}">{{ $emj_like }}</span>
                         <span class="emj_heart" ids="{{ $data->id }}">{{ $emj_heart }}</span>
                         <span class="emj_love" ids="{{ $data->id }}">{{ $emj_love }}</span>
                         <span class="emj_cry" ids="{{ $data->id }}">{{ $emj_cry }}</span>
                     </a>
